...
/*   Updated: 2021/05/13 22:55:50 by nargouse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || line == NULL || BUFFER_SIZE <= 0 || read(fd, NULL, 0) == -1)
		return (-1);
	*line = ft_strndup(left[fd] ? left[fd] : "",
			ft_strichr(left[fd] ? left[fd] : "", '\n'));
	if (ft_strchr(left[fd] ? left[fd] : "", '\n') != NULL)
		return (ft_if(&(left[fd]), &tmp));
	...
	*line = ft_strnjoin_free(*line, buf, ft_strichr(buf, '\n'));
	if (ret != BUFFER_SIZE && ft_strchr(buf, '\n') == NULL)
		return (ft_free((void **)&(left[fd])));
	tmp = left[fd];
	left[fd] = ft_strdup(ft_strchr(buf, '\n') + 1);
	free(tmp);
	return (1);
}
